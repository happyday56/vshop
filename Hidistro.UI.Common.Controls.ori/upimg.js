...
function InitUploader(uploaderId, uploadType,snailtype) {
    ...
    fileInput.onchange = function () {
        if (!RegExp("\.(jpg|gif|png|jpeg)$", "i").test(fileInput.value)) {
            ResetUploader(uploadType, uploaderId, snailtype);
            alert("\u8BF7\u9009\u62E9\u4E00\u4E2A\u56FE\u7247\u6587\u4EF6");
            return;
        }
        ...
        postForm.action = applicationPath + "/API/UpImg.ashx?action=upload&uploadType=" + uploadType + "&uploaderId=" + uploaderId + "&snailtype=" + snailtype;
        postForm.submit();
...
function ResetUploader(uploadType, uploaderId, snailtype) {
    var fileContent = document.getElementById(uploaderId + "_content");
    fileContent.removeChild(document.getElementById(uploaderId + "_file"));

    InitUploader(uploaderId, uploadType, snailtype);
}

// 上传回调
function UploadCallback(uploadType, uploaderId, uploadedImageUrl, snailtype) {
    ResetUploader(uploadType, uploaderId, snailtype);
...
// 错误回调
function ErrorCallback(uploadType, uploaderId, error, snailtype) {
    ResetUploader(uploadType, uploaderId, snailtype);
    alert(error);
}
