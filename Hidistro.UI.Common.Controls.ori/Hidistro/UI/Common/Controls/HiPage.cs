namespace Hidistro.UI.Common.Controls
{
    ...
    {
        private static readonly Regex AAZ0JeEma(2x58C7QQ)d9L4MH = new Regex("<div>(\\s+)<input type=\"hidden\" name=\"__VIEWSTATE\" id=\"__VIEWSTATE\" value=\"(?<data>.*?)\" />(\\s+)</div>(\\r\\n)+", RegexOptions.Compiled | RegexOptions.ExplicitCapture | RegexOptions.Multiline);
            ...
            else
            {
                using (StringWriter writer2 = new StringWriter())
                {
                    using (HtmlTextWriter writer3 = new HtmlTextWriter(writer2))
                    {
                        ...
                        Match match = AAZ0JeEma(2x58C7QQ)d9L4MH.Match(input);
                        if (match.Success)
                        {
