        ...
        protected VActivityidTemplatedWebControl()
        {
            if (MemberProcessor.GetCurrentMember() == null)
            {
                ...
                if (masterSettings.IsValidationService)
                {
                    ...
                    {
                        string str2 = this.AAZ0JeEma(2x58C7QQ)d9L4MH("https://api.weixin.qq.com/sns/oauth2/access_token?appid=" + masterSettings.WeixinAppId + "&secret=" + masterSettings.WeixinAppSecret + "&code=" + str + "&grant_type=authorization_code");
