namespace Hidistro.Core
{
    ...
    public static class Globals
    {
        ...
        public static int GetCurrentMemberUserId()
        {
            ...
            if ((cookie == null) || string.IsNullOrEmpty(cookie.Value))
            {
                //XTrace.WriteLine("当前登录的会员ID(GetCurrentMemberUserId)：0");
                return 0;
            }
            int result = 0;
            int.TryParse(cookie.Value, out result);
            //XTrace.WriteLine("当前登录的会员ID(GetCurrentMemberUserId)：" + result);
            return result;
        }
        ...
        {
            themeName = (themeName == null) ? SettingsManager.GetMasterSettings(true).VTheme : themeName;
            return (ApplicationPath + "/Templates/vshop/" + themeName).ToLower(CultureInfo.InvariantCulture);
        }
