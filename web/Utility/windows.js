...
windowPop=function(divWidth,divHeight,msgInfo,switchResult)
{
 ...
 this.autoClosePopAndReUrl = function (url,target) {
     setTimeout(function () {
         $("#popDivLock").remove();
             $("#windowsdialog").fadeOut(200, function () {
                 if (target == "parent") {
                     parent.location.href = url;
                 } else if (target == "frammain") {
                     parent.frammain.location.href = url;
                 } else {
                     $("#windowsdialog").fadeOut(200, function () { location.href = url; });
                 }
             });
     },  1500);
 }
...
function ShowMsgAndReUrl(msg, success,url,target) {
    if (success) {    //参数(宽value,高value,提示信息string,信息类别提示0,1,error,true,false,3,4)
        var popWin = new windowPop(350, 200, msg, 1);
        popWin.windoPopOpen();
        popWin.autoClosePopAndReUrl(url, target);
    }
    else {
        var popWin = new windowPop(350, 200, msg, 0);
        popWin.lockScreen();	//锁定背景
