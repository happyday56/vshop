<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" Inherits="Hidistro.UI.Web.Admin.ManageOrder" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="contentHolder" runat="server">
    ...
    <div class="dataarea mainwidth">
        ...
        <div class="searcharea clearfix br_search">
            <ul>
                ...
                <li><span>跨境订单：</span><span>
                    <abbr class="formselect">
                        <asp:DropDownList runat="server" ID="ddlIsCrossOrder" Width="107" />
                    </abbr>
                </span></li>
            ...
            <div class="batchHandleArea">
                <ul>
                    <li class="batchHandleButton"><span class="signicon"></span><span class="allSelect">
                        ...
                        <span class="printorder"><a href="javascript:printBatch()">快递单打印批次</a></span> <span
        ...
        <div class="datalist">
            <asp:DataList ID="dlstOrders" runat="server" DataKeyField="OrderId" Width="100%">
                <HeaderTemplate>
                    <table width="0" border="0" cellspacing="0">
                        <tr class="table_title">
                            <td width="24%" class="td_right td_left">昵称</td>
                            <td width="20%" class="td_right td_left">收货人</td>
                            <td width="110px" class="td_right td_left">支付方式</td>
                            <td width="14%" class="td_right td_left">订单实收款(元)</td>
                            <td width="160px" class="td_right td_left">订单状态</td>
                ...
                <ItemTemplate>
                    <tr class="td_bg">
                        <td>
                            <input name="CheckBoxGroup" type="checkbox" value='<%#Eval("OrderId") %>' />订单编号：<%#Eval("OrderId") %>&nbsp;(<%#Eval("OrderType").ToString() == "1" ? "<span style='color:#000000'>普通订单</span>" : "<span style='color:#ff0000'>开店订单</span>" %>)<asp:Literal ID="group" runat="server" Text='<%#  Eval("GroupBuyId")!=DBNull.Value&& Convert.ToInt32(Eval("GroupBuyId"))>0?"(团)":"" %>' /></td>
                        <td>提交时间：<Hi:FormatedTimeLabel ID="lblStartTimes" Time='<%#Eval("OrderDate") %>' ShopTime="true" runat="server"></Hi:FormatedTimeLabel></td>
                        <td><%# Eval("IsPrinted")!=DBNull.Value&&(bool)Eval("IsPrinted")?"已打印":"未打印" %></td>
                        <td><%#Eval("ReferralUserId").ToString()=="" ? "订单来源：主站" : Eval("ReferralUserId").ToString()=="0" ? "订单来源：主站" : "订单来源：" + Eval("StoreName")%></td>
                        <td>&nbsp;<%# String.IsNullOrEmpty(Eval("ShipOrderNumber").ToString()) ? "" : "物流单号：" + Eval("ShipOrderNumber")%></td>
                        <td align="right">&nbsp; <a href="javascript:RemarkOrder('<%#Eval("OrderId") %>','<%#Eval("OrderDate") %>','<%#Eval("OrderTotal") %>','<%#Eval("ManagerMark") %>','<%#Eval("ManagerRemark") %>');">
                            <Hi:OrderRemarkImage runat="server" DataField="ManagerMark" ID="OrderRemarkImageLink" /></a></td>
                    </tr>
                    <tr>
                        ...
                        <td>&nbsp;
                            <span class="Name" style="text-indent: 0px;">
                                <Hi:ImageLinkButton ID="lkbtnPayOrder" runat="server" Text="我已线下收款" CommandArgument='<%# Eval("OrderId") %>' CommandName="CONFIRM_PAY" OnClientClick="return ConfirmPayOrder()" Visible="false" ForeColor="Red"></Hi:ImageLinkButton><br />
                                <a href="javascript:CloseOrder('<%#Eval("OrderId") %>');">
                                    <asp:Literal runat="server" ID="litCloseOrder" Visible="false" Text="关闭订单" /></a>
                                <asp:Label CssClass="submit_faihuo" ID="lkbtnSendGoods" Visible="false" runat="server"><a style="color:Red;display:block;" href="javascript:DialogFrame('<%# "sales/SendOrderGoods.aspx?OrderId="+ Eval("OrderId") %>&reurl='+ encodeURIComponent(goUrl),'订单发货',null,null, function () { location.href=goUrl })">发货</a> </asp:Label>
                                <asp:Label ID="lkbtnSendLogistic" Visible="false" runat="server"><a style="color:Red;display:block;" href="javascript:DialogFrame('<%# "sales/SendSubOrderLogistic.aspx?OrderId="+ Eval("OrderId") %>&reurl='+ encodeURIComponent(goUrl),'录入物流单号',null,null, function () { location.href=goUrl })">录入物流单号</a> </asp:Label>
                                ...
                                <a style="color: Red"></a><a href="javascript:;" onclick="return CheckRefund(this.title)" runat="server" id="lkbtnCheckRefund" visible="false" title='<%# Eval("OrderId") %>'>确认退款</a> <a href="javascript:void(0)" onclick="return CheckReturn(this.title)" runat="server" id="lkbtnCheckReturn" visible="false" title='<%# Eval("OrderId") %>'>确认退货</a>
    ...
    <div id="RemarkOrder" style="display: none;">
        <div class="frame-content">
            ...
            <span class="frame-span frame-input100">标志：<em>*</em></span><Hi:OrderRemarkImageRadioButtonList runat="server" ID="orderRemarkImageForRemark" /><p>
                <span>备忘录：</span><asp:TextBox ID="txtRemark" TextMode="MultiLine" runat="server" Width="300" Height="50" />
            </p>
        </div>
    </div>
    ...
    <div id="CheckRefund" style="display: none;">
        <div class="frame-content">
            ...
            <div style="text-align: center;">
                <input type="button" id="Button2" onclick="javascript: acceptRefund();" class="submit_DAqueding" value="确认退款" />
                &nbsp;
                <input type="button" id="Button3" onclick="javascript: refuseRefund();" class="submit_DAqueding" value="拒绝退款" />
            </div>
        </div>
    </div>
    <!--确认退货--->
    <div id="CheckReturn" style="display: none;">
        <div class="frame-content">
            ...
            <br />
            <div style="text-align: center;">
                <input type="button" id="Button4" onclick="javascript: acceptReturn();" class="submit_DAqueding" value="确认退货" />&nbsp;
                <input type="button" id="Button5" onclick="javascript: refuseReturn();" class="submit_DAqueding" value="拒绝退货" />
            </div>
        </div>
    </div>
    <!--确认换货--->
    <div id="CheckReplace" style="display: none;">
        <div class="frame-content">
            ...
            <br />
            <div style="text-align: center;">
                <input type="button" id="Button6" onclick="javascript: acceptReplace();" class="submit_DAqueding" value="确认换货" />&nbsp;
                <input type="button" id="Button7" onclick="javascript: refuseReplace();" class="submit_DAqueding" value="拒绝换货" />
            </div>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        function printBatch() {
            ...
            if (orderIds == "") {
                ...
                var url = "sales/SetOrderPrintBatch.aspx?OrderIds=" + orderIds;
                DialogFrame(url, "设置打印订单批次", null, null);
            }
        }
