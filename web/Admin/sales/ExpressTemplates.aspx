     ...
     <UI:Grid ID="grdExpressTemplates" runat="server" ShowHeader="true" AutoGenerateColumns="false" DataKeyNames="ExpressId" HeaderStyle-CssClass="table_title" GridLines="None" Width="100%">
            <Columns>   
                ...
                <asp:TemplateField HeaderText="操作" ItemStyle-Width="40%" HeaderStyle-CssClass="td_left td_right_fff">
                    <ItemTemplate>
                    <span><a href='<%# "EditExpressTemplate.aspx?ExpressId=" + Eval("ExpressId") + "&ExpressName=" + Globals.UrlEncode((string)Eval("ExpressName")) + "&XmlFile=" + Eval("XmlFile")%>'>编辑</a></span>
                    <span><a href='<%# "AddSampleExpressTemplate.aspx?ExpressName=" + Globals.UrlEncode((string)Eval("ExpressName")) + "&XmlFile=" + Eval("XmlFile")%> '>添加相似单据</a></span>
                    <span><Hi:ImageLinkButton ID="lkDelete" Text="删除" IsShow="true"  CommandName="Delete" runat="server" /></span>    
                    </ItemTemplate>
                </asp:TemplateField>
            </Columns>
        </UI:Grid>
