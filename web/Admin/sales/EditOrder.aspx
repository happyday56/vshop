...
<div class="dataarea mainwidth databody ">
    ...
    <div class="list">
    <h1>商品列表</h1>    
      <UI:Grid ID="grdProducts" runat="server" AutoGenerateColumns="false" HeaderStyle-CssClass="table_title" DataKeyNames="SkuId" SortOrderBy="SkuId" SortOrder="DESC" CssClass="goods-list" GridLines="None" AllowSorting="true" Width="100%">
                
             <Columns>
                <asp:TemplateField HeaderText="商品" HeaderStyle-CssClass="td_right td_left">
                    <ItemTemplate>
                      <div style="float:left;width:60px;height:60px;"><a href='<%#"../../vshop/ProductDetails.aspx?ProductId="+Eval("ProductId") %>' target="_blank"><Hi:ListImage ID="HiImage2"  runat="server" DataField="ThumbnailsUrl" /></a></div>     
                      <div style="float:left;margin-left:10px;line-height:22px;"><span><a href='<%#"../../vshop/ProductDetails.aspx?ProductId="+Eval("ProductId") %>' target="_blank"><%# Eval("ItemDescription") %></a>
                ...
                <asp:TemplateField HeaderText="购买数量"  HeaderStyle-CssClass="td_right td_left"  HeaderStyle-Width="20%">
                       <itemtemplate> <%# Eval("Quantity") %>                    
                            <div style="display:none"><asp:TextBox  Columns="6" ID="txtQuantity" runat="server" Text='<%# Eval("Quantity") %>'  CssClass="forminput leftR10" TagPrice="inputValue"></asp:TextBox>  
            ...
            <asp:HyperLink ID="hlinkPurchase" runat="server" NavigateUrl='<%# string.Format(Globals.GetSiteUrls().UrlData.FormatUrl("FavourableDetails"),  Eval("PromotionId"))%>'
                Text='<%# Eval("PromotionName")%>' Target="_blank" Visible="false"></asp:HyperLink><%# Eval("PromotionName")%>
            </div>
                ...
                <asp:TemplateField HeaderText="操作" HeaderStyle-CssClass="td_left td_right_fff"  HeaderStyle-Width="15%" Visible="false">
...
<h1>订单实收款结算</h1>
        ...
        <table width="200" border="0" cellspacing="0">
          <tr style=" display:none">
            <td   align="right">打折优惠(元)：<br /></td>
            <td class="a_none"><span class="colorB"><Hi:FormatedMoneyLabel  ID="fullDiscountAmount" runat="server"  /></span></td>
            <td  class="a_none"><span class="Name"><asp:HyperLink Target="_blank" ID="lkbtnFullDiscount" runat="server" /></span></td>
          </tr>
          <tr style=" display:none">
            <td  style=" width:150px" >满额免费用活动(元)：</td>
            <td colspan="2" class="a_none"><asp:HyperLink Target="_blank" ID="lkbtnFullFree" runat="server" Text="0.00" /></td>
          </tr>
          <tr>
