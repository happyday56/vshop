namespace Hidistro.SqlDal.Orders
{
    ...
    public class SendNoteDao
    {
        ...
        public DbQueryResult GetAllSendNote(RefundApplyQuery query)
        {
            ...
            if (!string.IsNullOrEmpty(query.ExpressNo))
            {
                builder.AppendFormat(" AND ShipOrderNumber = '{0}' ", query.ExpressNo);
            }
            if (!string.IsNullOrEmpty(query.StartTime))
            {
                builder.AppendLine("      AND datediff(dd,'" + query.StartTime + "',ShippingDate)>=0");
            }
            if (!string.IsNullOrEmpty(query.EndTime))
            {
                builder.AppendLine("      AND datediff(dd,'" + query.EndTime + "',ShippingDate)<=0");
            }
            if (!string.IsNullOrEmpty(query.ShipTo))
            {
                builder.AppendLine("      AND ShipTo LIKE '%" + query.ShipTo + "%' ");
            }
