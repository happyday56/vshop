namespace Hidistro.SqlDal.Store
{
    ...
    public class PhotoGalleryDao
    {
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT count(*) FROM Hishop_PhotoGallery where CategoryId=0");
            return Convert.ToInt32(this.database.ExecuteScalar(sqlStringCommand));
        }

        public DataTable GetPhotoCategories()
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT *, (SELECT COUNT(PhotoId) FROM Hishop_PhotoGallery WHERE CategoryId = pc.CategoryId) AS PhotoCounts FROM Hishop_PhotoCategories pc ORDER BY DisplaySequence DESC");
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT count(*) FROM Hishop_PhotoGallery");
            return Convert.ToInt32(this.database.ExecuteScalar(sqlStringCommand));
        }
