namespace Hidistro.SqlDal.Commodities
{
    ...
    public class SkuDao
    {
        ...
        public SKUItem GetProductAndSku(MemberInfo currentMember, int productId, string options)
        {
            ...
            SKUItem item = null;
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(builder.ToString());
            //XTrace.WriteLine("--------------选择商品SKU时的SQL：" + builder.ToString() + "---(" + productId + ")");
            this.database.AddInParameter(sqlStringCommand, "ProductId", DbType.Int32, productId);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                if (reader.Read())
            ...
            //XTrace.WriteLine("--------------选择商品SKU后的结果：" + item.ToString());
            return item;
        }
        ...
        public SKUItem GetSkuItem(string skuId)
        {
            ...
            sql.AppendLine("SELECT sk.SkuId, sk.ProductId, sk.SKU, sk.Weight, CASE WHEN p.IsCross = 1 THEN p.MaxCross ELSE sk.Stock END AS Stock, sk.CostPrice, sk.Saleprice ");
            sql.AppendLine("FROM Hishop_SKUs AS sk LEFT OUTER JOIN Hishop_Products AS p ON sk.ProductId = p.ProductId ");
            sql.AppendLine("WHERE sk.SkuId=@SkuId; ");
