namespace Hidistro.SqlDal.Commodities
{
    ...
    public class CategoryDao
    {
        ...
        public int CreateCategory(CategoryInfo category)
        {
            ...
            if (!string.IsNullOrEmpty(category.CoverUrl))
            {
                this.database.AddInParameter(storedProcCommand, "CoverUrl", DbType.String, category.CoverUrl);
            }
            this.database.AddInParameter(storedProcCommand, "IsDisplayHome", DbType.Int32, category.IsDisplayHome);
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT CategoryId,Name,IconUrl,DisplaySequence,ParentCategoryId,Depth,[Path],RewriteName,HasChildren,FirstCommission,SecondCommission,ThirdCommission,CoverUrl, IsDisplayHome FROM Hishop_Categories ORDER BY DisplaySequence");
        ...
        public DataTable GetCategoriesByDisplayHome()
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT CategoryId,Name,IconUrl,DisplaySequence,ParentCategoryId,Depth,[Path],RewriteName,HasChildren,FirstCommission,SecondCommission,ThirdCommission,CoverUrl, IsDisplayHome FROM Hishop_Categories WHERE IsDisplayHome = 1 ORDER BY DisplaySequence");
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                return DataHelper.ConverDataReaderToDataTable(reader);
            }
        ...
        public CategoryActionStatus UpdateCategory(CategoryInfo category)
        {
            ...
            this.database.AddInParameter(sqlStringCommand, "CoverUrl", DbType.String, category.CoverUrl);
