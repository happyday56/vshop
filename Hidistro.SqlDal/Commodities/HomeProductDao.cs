namespace Hidistro.SqlDal.Commodities
{
    ...
    public class HomeProductDao
    {
        ...
        public DbQueryResult GetHomeProducts(MemberInfo member, ProductQuery query, bool isdistributor)
        {
            ...
            builder.Append("MainCategoryPath,ProductId, ProductCode,ShortDescription,ProductName,ShowSaleCounts, ThumbnailUrl60,ThumbnailUrl40,ThumbnailUrl100,ThumbnailUrl180,ThumbnailUrl220,ThumbnailUrl310, MarketPrice, ReviewsCount,");
            if (member != null)
            {
        ...
        public IList<ProductTwoInfo> GetProductList()
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT ProductId, ProductName FROM Hishop_Products WHERE SaleStatus = 1 AND IsDistributorBuy = 0 ");
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                return ReaderConvert.ReaderToList<ProductTwoInfo>(reader);
            }
        }
    }
}
