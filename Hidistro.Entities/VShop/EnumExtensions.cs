        ...
        public static string ToShowText(this Enum en, bool exceptionIfFail, string flagsSeparator)
        {
            ...
            if ((customAttributes != null) && (customAttributes.Length > 0))
            {
                ...
                foreach (string str4 in names)
                {
                    ...
                    if (num2 == 0L)
                    {
                        objArray2 = enumType.GetField(str4).GetCustomAttributes(typeof(EnumShowTextAttribute), false);
                    ...
                    else if ((num2 & num) == num2)
                    {
                        ...
                        if (objArray2.Length > 0)
                        {
                            builder.Append(((EnumShowTextAttribute) objArray2[0]).ShowText);
                        }
                        else
                        {
                            if (exceptionIfFail)
            ...
            objArray2 = field.GetCustomAttributes(typeof(EnumShowTextAttribute), false);
            if (objArray2.Length > 0)
            {
                str = ((EnumShowTextAttribute) objArray2[0]).ShowText;
