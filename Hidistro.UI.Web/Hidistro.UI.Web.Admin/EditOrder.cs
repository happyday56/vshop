...
namespace Hidistro.UI.Web.Admin
{
	...
	public class EditOrder : AdminPage
	{
		...
		private void btnUpdateOrderAmount_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				if (this.ValidateValues(out num, out num2, out num3))
				{
					...
					ValidationResults results = Validation.Validate<OrderInfo>(this.order, new string[]
					{
						"ValOrder"
					});
					if (!results.IsValid)
		...
		private void grdProducts_RowCommand(object sender, System.Web.UI.WebControls.GridViewCommandEventArgs e)
		{
			...
			else
			{
				if (e.CommandName == "setQuantity")
				{
					...
					System.Web.UI.WebControls.TextBox box = this.grdProducts.Rows[rowIndex].FindControl("txtQuantity") as System.Web.UI.WebControls.TextBox;
					...
					{
						if (num2 > OrderHelper.GetSkuStock(skuId))
						{
		...
		private bool ValidateValues(out decimal adjustedFreight, out decimal adjustedPayCharge, out decimal discountAmout)
		{
			...
			{
				str += Formatter.FormatErrorMessage("运费必须在0-1000万之间");
			}
			if (!decimal.TryParse(this.txtAdjustedPayCharge.Text.Trim(), out adjustedPayCharge))
			{
				str += Formatter.FormatErrorMessage("支付费用必须在0-1000万之间");
			}
			int length = 0;
			...
			{
				str += Formatter.FormatErrorMessage("订单折扣填写错误,订单折扣只能是数值，且不能超过2位小数");
			}
			bool result;
